...
def multipolyfit(xs, y, deg, full=False, model_out=False, powers_out=False):
    ...
    # powers = asarray(map(sum, itertools.combinations_with_replacement(generators, deg)))
    powers = asarray([sum(x) for x in itertools.combinations_with_replacement(generators, deg)])
...
def mk_model(beta, powers):
    ...
    def model(*args):
        ...
        xs = asarray((1,) + args)
        return sum([coeff * (xs**p).prod() for p, coeff in zip(powers, beta)])
    return model, beta, powers
